...
import com.gp.calendar.CalendarBean;
import com.gp.calendar.CalendarDTO;
import com.gp.calendar.CalendarFacade;
...
public class MainController extends HttpServlet {
  ...
  public static final String ADD_CAL_ENTRY = "ADD_CAL_ENTRY";
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    switch (action) {
      ...
      case USER_INFO_EDIT_ACTION:
        ...
        break;
      case ADD_CAL_ENTRY:
        responseJSON = addCalEntry(request);
        break;
      default:
  ...
  public JSONObject addCalEntry(HttpServletRequest request) {
    System.out.println(" Hey from calendar");
    ...
    if (!Helper.isValid(hour, date, businessId)) {
      responseJSON.put("status", "ERROR");
      responseJSON.put("message", "Missing or invalid attributes");
    ...
    if (user == null) {
      responseJSON.put("status", "ERROR");
      responseJSON.put("message", "Needs to login");
      return responseJSON;
    }
  ...
  private JSONObject userEdit(HttpServletRequest request) {
    ...
    if (dao.existDifferent(user.getUid(), username, ColumnName.USERNAME)
      ...
      responseJSON.put("status", "ERROR");
      responseJSON.put("message", "The username or email is already taken");
      responseJSON.put("action", "reload-slow");
      return responseJSON;
    }
  ...
  public JSONObject register(HttpServletRequest request) {
    ...
    if (!Helper.isValid(option, user.getEmail(), user.getFullName(), user.getPassword(),
        user.getUsername(), passConfirm)) {
    ...
    try {
      ...
      if (option.equals("opt-biz")) {
        ...
        valid = factory.createBusinessUser(bizName, user);
      } else if (option.equals("opt-cons")) {
        valid = factory.createConsumerUser(user);
      }
    } catch (Exception e) {
      e.printStackTrace();
      responseJSON.put("status", "ERROR");
  ...
  public JSONObject login(HttpServletRequest request) {
    ...
    if (user != null) {
      ...
      response.put("message", "Trying to create a duplicate account");
      return response;
    }
    UserDAO dao = new UserDAO("jdbc/gpappdb");
    ...
    if (!Helper.isValid(username, rawPassword)) {
      response.put("status", "ERROR");
      response.put("message", "Missing attributes");
      return response;
    }
    ...
    if (id == null) {
      response.put("status", "ERROR");
      response.put("message", "The provided credentials are invalid, user not found");
      return response;
    }
    user = dao.find(id);
  ...
  public JSONObject logout(HttpServletRequest request) {
    ...
    if (request.getSession().getAttribute("user") == null) {
      // System.out.println("User already exist");
      response.put("status", "ERROR");
      response.put("message", "Trying to logout when the user is not logged in");
      return response;
    }
